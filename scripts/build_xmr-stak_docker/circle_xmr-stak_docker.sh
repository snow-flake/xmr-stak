...
docker run --rm -it -v $HOME:/mnt/xmr-stak fedora:27 /bin/bash -c "
set -x ;
...
# Ubuntu (17.04)
########################
docker run --rm -it -v $HOME:/mnt/xmr-stak ubuntu:17.04 /bin/bash -c "
set -x ;
apt update -qq ;
...
# Ubuntu 16.04
########################
docker run --rm -it -v $HOME:/mnt/xmr-stak ubuntu:16.04 /bin/bash -c "
set -x ;
apt update -qq ;
...
# Ubuntu 14.04
########################
docker run --rm -it -v $HOME:/mnt/xmr-stak ubuntu:14.04 /bin/bash -c "
set -x ;
apt update -qq ;
...
docker run --rm -it -v $HOME:/mnt/xmr-stak centos:7 /bin/bash -c "
set -x ;
...
########################
# CUDA is not going to work on CentOS/RHEL beacuse it's only support gcc-4 in these distributions: http://docs.nvidia.com/cuda/cuda-installation-guide-linux/index.html
docker run --rm -it -v $HOME:/mnt/xmr-stak centos:6 /bin/bash -c "
set -x ;
yum install -y -q centos-release-scl epel-release ;
