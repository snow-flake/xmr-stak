...
void executor::eval_pool_choice() {
	...
	std::sort(eval_pools.begin(), eval_pools.end(),
			  [](jpsock *a, jpsock *b) { return b->get_pool_weight(true) < a->get_pool_weight(true); });
	...
	if (goal->get_pool_id() != xmrstak::globalStates::inst().pool_id) {
		...
		if (goal->is_logged_in()) {
			...
			if (prev_pool == nullptr || (!false && !false))
				reset_stats();

			if (false && (prev_pool != nullptr && !false))
		...
		std::sort(eval_pools.begin(), eval_pools.end(),
				  [](jpsock *a, jpsock *b) { return b->get_pool_weight(false) < a->get_pool_weight(false); });
...
void executor::ex_main() {
	...
	for (; i < pc; i++) {
		...
		if (!xmrstak::params::inst().poolURL.empty() && xmrstak::params::inst().poolURL == cfg.sPoolAddr) {
			...
			pools.emplace_back(i + 1, cfg.sPoolAddr, wallet, pwd, 9.9, false, params.poolUseTls, cfg.tls_fingerprint,
							   nicehash);
		} else
			pools.emplace_back(i + 1, cfg.sPoolAddr, cfg.sWalletAddr, cfg.sPasswd, cfg.weight, false, cfg.tls,
							   cfg.tls_fingerprint, cfg.nicehash);
	}

	if (!xmrstak::params::inst().poolURL.empty() && !already_have_cli_pool) {
		auto &params = xmrstak::params::inst();
		if (params.poolUsername.empty()) {
			printer::inst()->print_msg(L1, "ERROR: You didn't specify the username / wallet address for %s",
									   xmrstak::params::inst().poolURL.c_str());
			win_exit();
		}

		pools.emplace_back(i + 1, params.poolURL.c_str(), params.poolUsername.c_str(), params.poolPasswd.c_str(), 9.9,
						   false, params.poolUseTls, "", params.nicehashMode);
	}
...
bool executor::motd_filter_console(std::string &motd) {
	...
	motd.erase(std::remove_if(motd.begin(), motd.end(),
							  [](int chr) -> bool { return !((chr >= 0x20 && chr <= 0x7e) || chr == '\n'); }),
			   motd.end());
	return motd.size() > 0;
}
